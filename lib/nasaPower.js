...
async function syncPowerCache({ forceFull = false } = {}) {
  ...
  if (!forceFull && disk) {
    ...
    const seeded = trySeedFromTestDownloads();
    if (seeded) {
...
/**
 * Obtiene CSV listo para el cliente. Sirve caché fresca; refresca en background si está stale.
 * Primera carga sin caché: sync bloqueante (o seed local).
 */
async function getPowerDailyCsv({ refresh = false } = {}) {
  ...
  if (!refresh && disk) {
    memory = { at: Date.now(), csv: disk, meta };
    // Refresh en background si no hay sync en curso
    if (!syncPromise) {
  ...
  // Sin disco: intentar seed, luego sync bloqueante
  if (!disk) {
    const seeded = trySeedFromTestDownloads();
    if (seeded && !refresh) {
      if (!syncPromise) {
        syncPromise = syncPowerCache()
          .catch((e) => console.warn("[nasaPower] refresh tras seed falló:", e.message))
          .finally(() => {
            syncPromise = null;
          });
      }
      return { csv: seeded.csv, meta: seeded.meta, fromCache: true, refreshing: true };
    }
  }
...
function warmPowerCache() {
  getPowerDailyCsv()
    .then((r) => {
      console.log(
        `[nasaPower] warm OK (${r.meta?.rows ?? "?"} filas, source=${r.meta?.source || "?"})`
      );
    })
    .catch((e) => {
