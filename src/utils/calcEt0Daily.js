/**
 * ET0 diaria en mm/día según FAO-56 (Penman-Monteith),
 * usando DOY, T2M, RH2M, ALLSKY_SFC_SW_DWN, PS y WS10M.
 */
...
export function calcEt0Daily({
  ...
  const delta =
    (4098 * satVaporPressureKpa(Math.max(-50, Math.min(60, T)))) /
    Math.pow(T + 237.3, 2);

  const gamma = 0.000665 * P;

  const termRad =
    (0.408 * delta * Math.max(Rn - G, 0)) /
